...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen md:grid md:grid-cols-[240px_1fr]">
      {isDesktop ? (
        <aside className="border-r border-white/10 bg-white/5 p-6">
      ...
      <div className="flex min-h-screen flex-col">
        ...
        {!isDesktop ? (
          <nav className="fixed inset-x-0 bottom-0 flex border-t border-white/10 bg-[hsl(var(--background))]/95 px-3 py-3">
            {links.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
